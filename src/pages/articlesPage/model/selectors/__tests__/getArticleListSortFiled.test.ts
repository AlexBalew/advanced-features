...
describe('getArticleListSortField test', () => {
    test('getArticleListSortField return correct data', () => {
        ...
        expect(getArticleListSortField(state as StateSchema)).toEqual(
            state.articlesPage?.sortField,
        );
    });

    test("getArticleListSortField should return 'created at' field if there is no data in state", () => {
        const state: DeepPartial<StateSchema> = {};
